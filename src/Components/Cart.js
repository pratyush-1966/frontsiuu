...
const Cart = () => {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      const response = await axios.post("https://newbackend-production-9865.up.railway.app/api/v1/order", {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          const validateRes = await axios.post(
            "https://newbackend-production-9865.up.railway.app/api/v1/order/validate",
            body,
            {
              headers: {
                "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        ...
        const response = await axios.get('https://newbackend-production-9865.up.railway.app/api/v1/items', {
          headers: {
  ...
  const handleRemoveItem = async (id) => {
    try {
      ...
      await axios.delete(`https://newbackend-production-9865.up.railway.app/api/v1/items/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
